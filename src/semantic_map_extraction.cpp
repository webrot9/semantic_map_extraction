...
void SemanticNode::ObservationTopicSubscriber(
  ...
  //  // DEBUG // std::cerr << "Received POSX=" << observation->posx
  //  << ", POSY=" << observation->posy << ", THETA="; std::cerr <<
  //  observation->theta << ", DIMS (X,Y,Z) = (" << observation->dimx
  //  << ", " << observation->dimy << ", " << observation->dimz << ")
  //  "; std::cerr << "Converted image coords POSX=" << object_x << ",
  //  POSY=" << object_y << ", THETA="; std::cerr <<
  //  observation->theta << ", DIMS (X,Y,Z) = (" << dim_x << ", " <<
  //  dim_y << ", " << dim_z << ") " << std::endl; std::cerr <<
  //  "PARAMS = (" << observation->properties << ") " << std::endl;
...
int main(int argc, char** argv) {
  ...
  if (modality == 0) {
    sem_node = new SemanticNode(&n, robotname, path, StatXMLFilePath,
    ...
    sem_node = new SemanticNode(&n, src, robotname, path, StatXMLFilePath,
                                DynXMLFilePath, timeout, wait_service,
                                load_dyn_map);
